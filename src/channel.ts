...
const acpAgentPromptAdapter = {
  messageToolHints: (params: { cfg: any; accountId?: string | null }): string[] => {
    ...
    return [
      ...
      "- Send private/direct messages to other agents → use **acp_send_dm** tool (NOT the generic message tool)",
      ...
      "",
      "IMPORTANT: When sending a private message to another agent, ALWAYS use the `acp_send_dm` tool. Do NOT use the generic `message` tool for ACP DMs.",
      "",
      `When calling ACP tools (\`acp_send_dm\`, \`acp_group\`, \`acp_fetch_agent_md\`, \`acp_manage_contacts\`), you MUST ALWAYS pass your AID \`${selfAid}\` in the \`aid\` parameter. This is MANDATORY.`,
      ...
      "You have persistent context files (peer profiles, group info, memories) loaded into your system prompt.",
      "Use the `acp_context` tool to maintain these files:",
      "- After learning something important about a peer, call acp_context(action=\"update_peer\", section=\"Notes\", ...)",
      "- To record key events or decisions, call acp_context(action=\"append_memory\", scope=\"peer|group|identity\", ...)",
      "- To look up past memories, call acp_context(action=\"search_memory\", scope=..., query=...)",
      "Keep context files concise and up-to-date. Only record genuinely important information.",
    ];
  },
};

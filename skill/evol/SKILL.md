...
1. 先判断是单身份还是多身份：
   - 单身份：读取 `channels.evol.agentMdPath`
   - 多身份：先确定目标 `accountId`，再读取 `channels.evol.identities.{accountId}.agentMdPath`
...
编辑 `~/.openclaw/openclaw.json` 中 `channels.evol` 字段（用 Read + Edit 深度合并，保留其他字段）：

- 先判定配置模式：
  - 多身份：`channels.evol.identities` 非空对象
  - 单身份：存在 `channels.evol.agentName` 且 `identities` 不存在/为空

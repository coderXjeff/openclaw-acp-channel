...
| 配置项 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| `maxTurns` | number | `15` | 单个会话最大入站消息次数 |
| `maxDurationMs` | number | `600000` | 会话最大持续时间（毫秒），默认 10 分钟 |
| `idleTimeoutMs` | number | `120000` | 空闲超时时间（毫秒），默认 2 分钟 |
...
// 只允许特定 Agent
"allowFrom": ["alice.aid.pub", "bob.aid.pub"]
```

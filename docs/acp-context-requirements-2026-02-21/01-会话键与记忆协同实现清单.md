...
2. Group 会话采用“identity + groupId 稳定粒度”，Duty 也并入该键。  
...
1. `handleDutyDispatchForIdentity` 生成会话键时改为 `buildGroupSessionKey`。  
2. Duty Prompt 仍保留（行为层隔离），但会话桶与群聊共享。  
3. Duty 相关记忆直接落到 `groups/{gid}/MEMORY.md`。
...
4. Duty 与 Group 使用同一个会话键，monitor 与 session-rating 的 key 生成逻辑完全一致。
